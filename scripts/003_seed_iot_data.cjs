;(async () => {
  ...
  try {
    // Use centralized admin client wrapper (server-only)
    ...
    if (testUserError) {
      console.error('Test user creation error:', testUserError.message || testUserError)
    } else {
      ...
      if (devicesError) {
        console.error('Devices creation error:', devicesError.message || devicesError)
      } else {
        ...
        if (devicesData) {
          ...
          if (sensorError) {
            console.error('Sensor data creation error:', sensorError.message || sensorError)
          } else {
          ...
          if (settingsError) {
            console.error('Settings creation error:', settingsError.message || settingsError)
          } else {
          ...
          if (alertsError) {
            console.error('Alerts creation error:', alertsError.message || alertsError)
          } else {

/*
Program Version 0.8 (Updated for Pi MQTT and Supabase Integration)
Date 2026/01/28
- เปลี่ยน MQTT Server เป็น 192.168.1.104 (Pi Local IP)
- เพิ่ม DEVICE_ID และ topic_prefix
- ตั้งค่า Topic เป็น iot/Station_1/ammonia
- เพิ่ม id field ใน JSON payload
- แก้ไขการอ่านค่า DHT22 ให้ทำงานถูกต้อง
...
char mqtt_server[40] = "192.168.1.104";  // Local IP ของ Pi
...
void measureAndSendData() {
  ...
  if (!tempReadSuccess) {
    // ถ้าอ่านไม่สำเร็จ ใช้ค่าล่าสุดที่บันทึกไว้
